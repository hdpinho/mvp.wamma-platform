...
| Entidad | Campos clave | Notas |
|---|---|---|
| `persona` | id, nombre_apellido, cedula, telefono_whatsapp, telefonos_adicionales, correo, canal_origen | **Raíz del ámbito comercial.** Deduplicada por cédula; por **cualquiera de sus teléfonos** si la cédula falta. El teléfono principal es el último que dio el cliente; los demás se conservan. Distinta de `usuario` (001) y de `solicitante` (WMA-F-FIN-001): una persona puede no llegar a ser nunca ninguna de las dos. Cédula, teléfonos y correo cifrados + índice ciego |
| `fusion_persona` | persona_sobreviviente_id, copia_absorbida, oportunidad_ids, cita_ids, interaccion_ids, fusionada_en | **Append-only.** Copia íntegra del registro absorbido y de lo que se le reasignó. Sin ella la fusión sería irreversible (`010-crm-comercial/plan.md` §5.1) |
| `oportunidad` | id, persona_id, vehiculo_id, etapa, modalidad_pago, valor_estimado + moneda + tasa_bcv, asesor_id, proxima_accion, proxima_accion_fecha, motivo_perdida, solicitud_credito_id, enlace_financiamiento (token, emitido_en) | Intención de compra sobre un vehículo. `motivo_perdida` obligatorio si y solo si `etapa = cerrado_perdido`. Los estados cerrados son terminales: retomar contacto crea una oportunidad nueva. El enlace personal de solicitud de crédito se emite al vender con financiamiento (`010-crm-comercial/spec.md` §8.8) |
